...
out = 'ann_paper'
print('Tables 1, 2 and 3 in progress')
table1 = pd.DataFrame()
table2 = pd.DataFrame()
table3 = pd.DataFrame()
artificials = [f'Artificial metagenome #{i}' for i in range(1, 6)]
folders = [f'{out}{folder}' for folder in ['', '/first_group', '/third_group', '/fourth_group', '/fifth_group']]
for i in range(len(folders)):
    upimapi_res = pd.read_csv(f'{folders[i]}/upimapi_genomes/UPIMAPI_results.tsv', sep='\t')
    upimapi_res = upimapi_res.groupby('qseqid')[
        ['sseqid', 'Protein names', 'Cross-reference (KEGG)', 'EC number',
         'Cross-reference (eggNOG)']].first().reset_index()
    n_proteins = count_on_file('>', f'{folders[i]}/genes_trimmed.fasta')
    ...
    partial_table1 = pd.DataFrame([
        ['Genes identified after gene calling', n_proteins],
    ...
    partial_table1.columns = [artificials[i]]
    table1 = pd.concat([table1, partial_table1], axis=1)
    print(table1)
    # Table 2
    recognizer_res = pd.read_csv(f'{folders[i]}/recognizer_genomes/reCOGnizer_results.tsv', sep='\t')
    recognizer_res = recognizer_res[recognizer_res['evalue'] < 0.001]
    cog_res = recognizer_res[recognizer_res['DB ID'].str.startswith('COG')]
    partial_table2 = pd.merge(
        upimapi_res[upimapi_res['Protein names'] == 'Uncharacterized protein']['qseqid'], cog_res, on='qseqid',
        how='left')
    no_cog_id = partial_table2.groupby('qseqid')['cog'].first().reset_index()['cog'].isnull().sum()
    partial_table2 = partial_table2.groupby(['COG general functional category', 'COG functional category'])[
        'COG functional category'].count()
    partial_table2 = partial_table2.append(pd.Series({('No COG ID', '-'): no_cog_id}))
    table2 = pd.concat([table2, partial_table2], axis=1)
    print(table2)
    # Table 3
    partial_table3 = []
    for db in ['CDD', 'NCBIfam', 'Protein_Clusters', 'TIGRFAM', 'Pfam', 'Smart', 'COG', 'KOG']:
        ...
        line = {'Database': db,
                f'Annotated proteins #{i+1}': percentage(len(df), n_proteins)}
        if db not in ['Smart', 'KOG']:
            line[f'Proteins with assigned EC number #{i+1}'] = percentage(df["EC number"].notnull().sum(), n_proteins)
        else:
            line[f'Proteins with assigned EC number #{i+1}'] = '0 (0 %)'
        partial_table3.append(line)
    table3 = pd.concat([table3, pd.DataFrame(partial_table3)], axis=1)
    print(table3)

out = 'ann_paper'
table1.to_excel(f'{out}/table1.xlsx')
table2.columns = artificials
table2.fillna(value=0.0).astype(int).to_excel(f'{out}/table2.xlsx')
table3.to_excel(f'{out}/table3.xlsx', index=False)
...
for folder in folders:
    ...
    partial = rec_res.loc[
        [ide for ide in up_res[up_res['Protein names'] == 'Uncharacterized protein']['qseqid'] if ide in rec_res.index]]
    t = len(partial) - partial['COG general functional category'].isnull().sum() + partial[
        'COG functional category (letter)'].isin(['R', 'S']).sum()
            ...
            for col in df.columns:
                if col not in ['Qualifier', 'Tool']:
                    if col in ['TPs', 'FPs', 'FNs']:
                        table4_info.at[j, col] += [
                            df.iloc[j][col] / (df.iloc[j]['TPs'] + df.iloc[j]['FPs'] + df.iloc[j]['FNs'])]
                    elif col in ['# of total identifications']:
                        n_p = n_proteins_dfast if df.iloc[j]['Tool'] == 'DFAST' else (
                            n_proteins_prokka if df.iloc[j]['Tool'] == 'Prokka' else n_proteins)
...
for j in df.index:
    for col in table4_info.columns:
        if col not in ['Qualifier', 'Tool']:
            table4.at[
                j, col] = f'{round(np.mean(table4_info.iloc[j][col]) * 100, 2)} +- {round(np.std(table4_info.iloc[j][col]) * 100, 2)} %'
...
prokka_real.rename(columns={'EC_number': 'EC number (Prokka)', 'COG': 'COG (Prokka)', 'locus_tag': 'ID (Prokka)'},
                   inplace=True)
...
real_df = pd.merge(real_df, eggnog_real[['#query', 'EC', 'eggNOG_OGs']], left_on='qseqid', right_on='#query',
                   how='left')
...
real_df['EC number (UPIMAPI + reCOGnizer)'] = real_df['EC number (UPIMAPI)'].combine_first(
    real_df['EC number (reCOGnizer)'])
...
pd.read_csv(f'{out}/upimapi_genomes/UPIMAPI_results.tsv', sep='\t').iloc[:50].to_csv(f'{out}/Table S5.tsv', sep='\t',
                                                                                     index=False)
...
n_proteins_prokka = [count_on_file('>', f'{out}{iterations[i]}/prokka_genomes/PROKKA_{prokka_dates[i]}.faa') for i in
                     range(len(iterations))]
n_proteins_dfast = [count_on_file('>', f'{out}{iterations[i]}/dfast_genomes/protein.faa') for i in
                    range(len(iterations))]
n_ids_u = [len(set(pd.read_csv(f'{out}{iterations[i]}/upimapi_genomes/UPIMAPI_results.tsv', sep='\t')['qseqid'])) for i
           in range(len(iterations))]
n_ids_r = [len(set(pd.read_csv(f'{out}{iterations[i]}/recognizer_genomes/reCOGnizer_results.tsv', sep='\t')['qseqid']))
           for i in range(len(iterations))]
n_ids_u_p_r = [len(set(
    list(pd.read_csv(f'{out}{iterations[i]}/upimapi_genomes/UPIMAPI_results.tsv', sep='\t')['qseqid']) + list(
        pd.read_csv(f'{out}{iterations[i]}/recognizer_genomes/reCOGnizer_results.tsv', sep='\t')['qseqid']))) for i in
               range(len(iterations))]
n_ids_e = [len(set(
    pd.read_csv(f'{out}{iterations[i]}/eggnog_mapper_genomes/eggnog_results.emapper.annotations', sep='\t', skiprows=4,
                skipfooter=3)['#query'])) for i in range(len(iterations))]
n_ids_m = [len(set(parse_mantis_consensus(f'{out}{iterations[i]}/mantis_genomes/consensus_annotation.tsv')['Query']))
           for i in range(len(iterations))]
n_ids_p = []
for i in range(len(iterations)):
...
for t in [(n_ids_u, n_proteins), (n_ids_r, n_proteins), (n_ids_e, n_proteins), (n_ids_m, n_proteins),
          (n_ids_p, n_proteins_prokka), (n_ids_d, n_proteins_dfast)]:
