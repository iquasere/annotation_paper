from paper_utils import parse_mantis_consensus, calculate_quality_metrics, parse_blast, blast_consensus, parse_gff
import pandas as pd
import numpy as np
...
upimapi_genomes = upimapi_genomes.groupby('Entry')[['EC number', 'Cross-reference (eggNOG)']].first().reset_index()
...
for col in ['#query', 'Query']:
    del res_df[col]
# Prokka and DFAST have their own gene callings
prokka_df = pd.merge(blast_consensus(f'{out}/prokka_genomes/PROKKA_01032022.blast'), uniprotinfo, on='Entry', how='left')
prokka_df = pd.merge(prokka_df, pd.read_csv('ann_paper/prokka_genomes/PROKKA_01032022.tsv', sep='\t')[
    ['locus_tag', 'EC_number', 'COG']], left_on='qseqid', right_on='locus_tag', how='left')
prokka_df.rename(
...
prokka_df = df_post_processing(prokka_df)
dfast_df = df_post_processing(dfast_df)
quality_benchmark = []
for fide in ['EC number', 'COG']:
    for ftool in ['eggNOG mapper', 'mantis', 'UPIMAPI + reCOGnizer']:
        quality_benchmark = add_to_quality_benchmark(quality_benchmark, res_df, fide, ftool)
    add_to_quality_benchmark(quality_benchmark, prokka_df, fide, 'Prokka')
    add_to_quality_benchmark(quality_benchmark, dfast_df, fide, 'DFAST')
pd.DataFrame(quality_benchmark).to_excel(f'{out}/tools_comparison.xlsx', index=False)
...
for fide in ['EC number', 'COG']:
    ...
    heatmap_df = pd.merge(
        heatmap_df, counts_funcs_series(prokka_df, f'{fide} (Prokka)'), left_index=True, right_index=True, how='outer')
    heatmap_df = pd.merge(
        heatmap_df, counts_funcs_series(dfast_df, f'{fide} (DFAST)'), left_index=True, right_index=True, how='outer')
